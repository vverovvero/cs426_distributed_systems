...
// #define CHECKPOINT_DISK_OFFSET (2147483648) //offset in bytes, is size of log (first 2GB)
// #define CHECKPOINT_SIZE (8589934592) //in bytes, 8GB total size.  divides into 2097152 segments of 4096 bytes

// #define CH_BLOCK_SIZE (4096) //4096 bytes.  divides into 64 segments of 512 segments of 8-bytes
// #define CH_BLOCK_NUM_SLOTS (511) //how many uint64_t fit in one block, excluding the checksum
...
// //load a block of size 4096 bytes
// void * ch_load_block(){
// 	void * addr = mmap(NULL, CH_BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
// 	int munmap_return = munmap(addr, CH_BLOCK_SIZE);
// 	assert(munmap_return == 0);
// }

// void ch_synchronize(void *dest, const void *source){
// 	memcpy(dest, source, CH_BLOCK_SIZE);
// 	msync(dest, CH_BLOCK_SIZE, MS_SYNC);
// }

// //for a block, calculate checksum
...
//   word++;
//   uint64_t checksum = 0;
//   //iterate over slots in a block
//   for(i=0; i<CH_BLOCK_NUM_SLOTS; i++){
...
// //given mmapped addr
// // void ch_write_superblock(void *addr, uint32_t generation, uint32_t start, uint32_t size){
// //   Ch_superblock ch_superblock;
// //   ch_superblock.generation = generation;
// //   ch_superblock.start = start; //starts at first block
// //   ch_superblock.size = size; //initially, the log is size 0

// //   synchronize(addr, &ch_superblock);

// //   //perform checksum
// //   ch_superblock.checksum = set_checksum(addr);
// //   synchronize(addr, &ch_superblock);
// // }
...
// 		Ch_block *ch_block = (Ch_block *) ch_load_block();
...
// 	assert(lseek_return != -1);
// 	//Copy information from struct into the disk
// 	ssize_t write_return = write(fd, addr, 4096);
// 	printf("write_return: %d\n", write_return);  //2147479552 (why is it only 2GB?)
// 	assert(write_return == 4096);
// 	printf("Finished writing to the disk\n");
// }
...
int dump_checkpoint(int fd, Graph *graph, uint32_t generation){
	...
	set_checkpoint_num_slots(serial_size - 1);
	//Load virtual memory
